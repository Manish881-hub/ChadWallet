  ...
  return (
    <div className="bg-[#111111] border border-[#1F1F1F] rounded-xl flex-1 flex flex-col overflow-hidden min-h-[300px]">
        ...
        <div className="flex gap-2">
          {(['Recent', 'Open', 'Closed'] as PositionsTab[]).map(t => (
            <button 
              key={t} 
              onClick={() => setTab(t)} 
              className={`text-[11px] font-semibold flex items-center gap-1.5 transition-colors ${tab === t ? 'text-white' : 'text-[#6B7280] hover:text-[#A0A0A0]'}`}
            >
      ...
      <div className="flex items-center px-4 py-2 border-b border-[#1F1F1F]/50">
        ...
        <span className="text-[11px] text-[#6B7280] font-mono w-[15%] text-right hidden sm:block">Avg Entry</span>
        <span className="text-[11px] text-[#6B7280] font-mono w-[15%] text-right hidden sm:block">Current</span>
        <span className="text-[11px] text-[#6B7280] font-mono w-[20%] text-right">PnL</span>
        <span className="text-[11px] text-[#6B7280] font-mono w-[20%] text-right">Value</span>
        <span className="text-[11px] text-[#6B7280] font-mono w-[10%] text-right"></span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {!authenticated ? (
           <div className="h-full flex flex-col items-center justify-center gap-2 opacity-50 p-6">
             ...
             <span className="text-sm font-mono text-[#555]">Connect wallet to view positions</span>
           </div>
        ) : filteredPositions.length === 0 ? (
           <div className="h-full flex flex-col items-center justify-center gap-2 opacity-50 p-6">
             <span className="text-sm font-mono text-[#555]">No {tab.toLowerCase()} positions</span>
           </div>
        ) : (
             ...
             {filteredPositions.map(pos => (
               <div key={pos.id} className="flex items-center px-4 py-2.5 border-b border-[#1F1F1F]/50 hover:bg-white/5 transition-colors group">
                 ...
                 <div className="w-[10%] text-right flex justify-end">
                   <button className="opacity-0 group-hover:opacity-100 p-1 hover:bg-white/10 rounded transition-all" title="Trade">
