  ...
  return (
    <div className="bg-[#111111] border border-[#1F1F1F] rounded-xl h-full flex flex-col overflow-hidden min-h-[300px]">
      <div className="flex items-center gap-4 px-4 py-3 border-b border-[#1F1F1F]">
        {(['All swaps', 'Buys', 'Sells'] as SwapsTab[]).map(t => (
          <button 
            key={t} 
            onClick={() => setTab(t)} 
            className={`text-[13px] font-bold transition-colors relative ${tab === t ? 'text-white' : 'text-[#9CA3AF] hover:text-[#A0A0A0]'}`}
          >
            {t}
            {tab === t && (
      ...
      <div className="flex items-center px-4 py-2 border-b border-[#1F1F1F]/50">
         ...
         <span className="text-[11px] text-[#9CA3AF] font-mono w-[15%]">Action</span>
         <span className="text-[11px] text-[#9CA3AF] font-mono w-[25%] text-right">Amount</span>
         <span className="text-[11px] text-[#9CA3AF] font-mono w-[20%] text-right">MCap</span>
         <span className="text-[11px] text-[#9CA3AF] font-mono w-[20%] text-right">Time</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {!authenticated ? (
          <div className="h-full flex flex-col items-center justify-center gap-2 opacity-50 p-6">
            ...
            <span className="text-sm font-mono text-[#888]">Connect wallet to view trades</span>
          </div>
          ...
          <div className="flex flex-col">
            {filteredTrades.map(trade => (
              <Link key={trade.id} href={`/trade/So11111111111111111111111111111111111111112`} className="flex items-center px-4 py-2.5 border-b border-[#1F1F1F]/50 hover:bg-white/5 transition-colors cursor-pointer group">
                ...
                <div className="w-[20%] text-right flex items-center justify-end gap-2">
                  <span className="text-[11px] font-mono text-[#888] group-hover:text-[#A0A0A0] transition-colors">{trade.time}</span>
